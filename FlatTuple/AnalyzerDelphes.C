...
void AnalyzerDelphes::Loop(const string modeStr, const string outFileName)
{
...
//by  b_branchname->GetEntry(ientry); //read only this branch
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    for ( size_t j1 : jetIdxs ) {
      ...
      const int nbjetsInLepT = (jets_bTag[j1] == 1) ? 1 : 0;
      for ( size_t j2 : jetIdxs ) {
        ...
        for ( size_t j3 : jetIdxs ) {
          ...
          for ( size_t j4 : jetIdxs ) {
            ...
            int nbjetsInHadT = 0;
            if ( jets_bTag[j2] ) ++nbjetsInHadT;
            if ( jets_bTag[j3] ) ++nbjetsInHadT;
            if ( jets_bTag[j4] ) ++nbjetsInHadT;
            if ( mode == Mode::TT ) {
