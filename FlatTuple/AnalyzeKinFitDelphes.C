...
//by  b_branchname->GetEntry(ientry); //read only this branch
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if ( gen_lep.Pt()   > 0 and gen_lep.DeltaR(sol_lepP4)    < 0.1 ) b_genMatch += 100000;
    if ( gen_nu.Pt()    > 0 and gen_nu.DeltaR(sol_nuP4)      < 0.1 ) b_genMatch += 10000;
    if ( gen_lepB.Pt()  > 0 and gen_lepB.DeltaR(sol_ljP4)    < 0.1 ) b_genMatch += 1000;
    if ( gen_hadJ1.Pt() > 0 and (gen_hadJ1.DeltaR(sol_wj1P4) < 0.1 or gen_hadJ1.DeltaR(sol_wj2P4) < 0.1) ) b_genMatch += 100;
    if ( gen_hadJ2.Pt() > 0 and (gen_hadJ2.DeltaR(sol_wj1P4) < 0.1 or gen_hadJ2.DeltaR(sol_wj2P4) < 0.1) ) b_genMatch += 10;
    if ( gen_hadB.Pt()  > 0 and gen_hadB.DeltaR(sol_hbP4)    < 0.1 ) b_genMatch += 1;
