#define AnalyzeKinFitHYTuple_cxx
...
void AnalyzeKinFitHYTuple::Loop(const string modeStr, const string outFileName)
{
  //   In a ROOT session, you can do:
  //      root> .L AnalyzeKinFitHYTuple.C
  //      root> AnalyzeKinFitHYTuple t
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    const auto jets_pt = *jet_pT, jets_eta = *jet_eta, jets_dphi = *jet_phi, jets_e = *jet_E;
    ...
    for ( size_t j1 : jetIdxs ) {
      jetP4s[0].SetPtEtaPhiE(jets_pt[j1], jets_eta[j1], jets_dphi[j1], jets_e[j1]);
      const int nbjetsInLepT = (jets_CSV[j1] > CSVM) ? 1 : 0;
      for ( size_t j2 : jetIdxs ) {
        if ( j2 == j1 ) continue;
        jetP4s[1].SetPtEtaPhiE(jets_pt[j2], jets_eta[j2], jets_dphi[j2], jets_e[j2]);
        for ( size_t j3 : jetIdxs ) {
          if ( j2 >= j3 ) continue;
          if ( j3 == j1 ) continue;
          jetP4s[2].SetPtEtaPhiE(jets_pt[j3], jets_eta[j3], jets_dphi[j3], jets_e[j3]);
          for ( size_t j4 : jetIdxs ) {
            ...
            jetP4s[3].SetPtEtaPhiE(jets_pt[j4], jets_eta[j4], jets_dphi[j4], jets_e[j4]);
